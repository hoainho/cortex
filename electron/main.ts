...
CRITICAL: Nếu bạn trả lời mà KHÔNG gọi cortex_perplexity_search hoặc cortex_perplexity_read_url, đó là SAI. LUÔN gọi tool trước.`
        ...
        const isToolOnlyQuery = hasImageKeyword || isImagePrompt
        if (isToolOnlyQuery) {
...
Return ONLY the enhanced prompt, nothing else.`
          ...
          try {
            ...
            if (!toolResult.isError) {
              ...
              if (imagePathMatch) {
                ...
                finalResponse = toolResult.content
              }

              emitThinking('tool_call', 'done', 'Image generated', `${finalResponse.length} chars`)
              mainWindow?.webContents.send('chat:stream', { conversationId, content: finalResponse, done: true })
              persistAssistantResponse(conversationId, finalResponse)

              await stageAfterHooks({
                projectId, conversationId, query, response: finalResponse,
                model: 'direct-image-gen', metadata: { path: 'direct_image' }
              }, emitThinking)

              return { success: true, content: finalResponse, contextChunks: [] }
            }

            emitThinking('tool_call', 'error', 'Image generation failed', toolResult.content)
            mainWindow?.webContents.send('chat:stream', { conversationId, content: `⚠️ ${toolResult.content}`, done: true })
            return { success: false, error: toolResult.content }
          } catch (imgErr) {
            const errMsg = imgErr instanceof Error ? imgErr.message : String(imgErr)
            console.error('[Chat] Direct image gen failed:', errMsg)
            emitThinking('tool_call', 'error', 'Image generation failed', errMsg)
            mainWindow?.webContents.send('chat:stream', { conversationId, content: `⚠️ Image generation failed: ${errMsg}`, done: true })
        ...
        if (generatedImageMarkdown) {
          const imgPath = generatedImageMarkdown.replace('CORTEX_GENERATED_IMAGE:', '')
          console.log(`[Chat] Injecting generated image into response: ${imgPath}`)
          try {
            const imgBuf = readFileSync(imgPath)
            const imgB64 = imgBuf.toString('base64')
            const sizeKB = Math.round(imgBuf.length / 1024)
